...
function table_row_changed(data) {
  if (data && data.length > 0) {
    ...
    window.noiseMeters.forEach(function (noiseMeter) {
...
function fill_data() {
  ...
  var last_calibration_date = document.getElementById("inputLastCalibrationDate");
...
function handle_update() {
  ...
  fetch("".concat(baseUri, "/noise_meters/").concat(window.noiseMeter.id), {
    method: "PATCH",
    headers: {
...
function handle_create() {
  ...
  var formData = new FormData(form);
  fetch("".concat(baseUri, "/noise_meters/"), {
    method: "POST",
    headers: {
      "X-CSRF-TOKEN": csrfToken,
      Accept: "application/json",
