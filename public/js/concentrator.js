...
function table_row_changed(data) {
  if (data && data.length > 0) {
    ...
    window.concentrators.forEach(function (concentrator) {
...
function fill_data() {
  ...
  var inputLastCommunicationPacketSent = document.getElementById("inputLastCommunicationPacketSent");
  var inputLastAssignedIpAddress = document.getElementById("inputLastAssignedIpAddress");
  ...
  if (window.modalType == "update") {
    ...
    inputLastCommunicationPacketSent.value = window.concentrator.last_communication_packet_sent;
    inputLastAssignedIpAddress.value = window.concentrator.last_assigned_ip_address;
...
function handle_update() {
  ...
  fetch("".concat(baseUri, "/concentrators/").concat(window.concentrator.id), {
    method: "PATCH",
    headers: {
...
function handle_create() {
  ...
  var formData = new FormData(form);
  fetch("".concat(baseUri, "/concentrators/"), {
    method: "POST",
    headers: {
      "X-CSRF-TOKEN": csrfToken,
      Accept: "application/json",
