...
class PartialsReportChart extends Component
{
    ...
    public function __construct(
        ...
        // Use pre-fetched data if available (MUCH faster!)
    ...
    {
        // Check if we have pre-fetched data (from controller)
        $dateKey = $this->date->format('Y-m-d');
        
        if (!empty($this->preparedData) && isset($this->preparedData[$dateKey])) {
            // ✅ USE PRE-FETCHED DATA - No database query!
            return $this->getNoiseDataFromPrepared($dateKey);
        }
        
        // ⚠️ FALLBACK: Query database (slower, only if preparedData not available)
        return $this->getNoiseDataFromDatabase();
    }
    
    /**
     * Get noise data from pre-fetched prepared data (FAST!)
     */
    private function getNoiseDataFromPrepared(string $dateKey): array
    {
        $dayData = $this->preparedData[$dateKey];
        $noiseData = [];
        
        // Access the 'slots' array directly
        if (isset($dayData['slots']) && is_array($dayData['slots'])) {
            foreach ($dayData['slots'] as $timestamp => $slotData) {
                // Only include data points that exist (not '-')
                if (isset($slotData['exists']) && $slotData['exists'] && isset($slotData['leq_data'])) {
                    $noiseData[] = [
                        'x' => $timestamp,
                        'y' => (float) $slotData['leq_data']
                    ];
                }
            }
        }
        
        // Already in chronological order from preparedData
        return $noiseData;
    }
    
    /**
     * Get noise data from database (SLOW - fallback only)
     */
    private function getNoiseDataFromDatabase(): array
    {
