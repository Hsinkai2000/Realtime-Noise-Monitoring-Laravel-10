...
class SoundLimit extends Model
{
    ...
    public function leq1h_limit($last_data_datetime)
    {
        ...
        // Get the limit from 12h table (1h limits are stored in 12h table)
        $limit = $this->sound_limits_values_12hr()[$day][$time_map];
        
        // If limit is missing or very high (>=120), fallback to Mon-Sat limit
        if ($limit === null || $limit >= 120) {
            $monSatLimit = $this->sound_limits_values_12hr()['mon_sat'][$time_map];
            if ($monSatLimit !== null && $monSatLimit < 120) {
                return $monSatLimit;
            }
        }
    ...
    public function check_12_1_hour_limit_type($last_data_datetime)
    {
        ...
        // Use 1-hour limits for evening/night periods (7pm-7am) for all categories
        // This ensures proper dose calculation for all days including Sunday/PH and Monday morning
        if ($time_map != 0) {
